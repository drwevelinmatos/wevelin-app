...
========================= */
// =========================
// MRPA — gerar tabela compacta (1 página) + imprimir em janela nova
// =========================
function buildMRPARows(){
  const tbody = document.getElementById('mrpa-tbody');
  if(!tbody) return;

  const rows = [];

  // Dia 1 (clínica): 2 medições
  rows.push({dia:'1', periodo:'Clínica', med:'1', hora:'', pas:'', pad:'', pulso:''});
  rows.push({dia:'1', periodo:'Clínica', med:'2', hora:'', pas:'', pad:'', pulso:''});

  // Dias 2 a 5: 3 manhã + 3 noite
  for(let d=2; d<=5; d++){
    for(let i=1;i<=3;i++){
      rows.push({dia:String(d), periodo:'Manhã', med:String(i), hora:'', pas:'', pad:'', pulso:''});
    }
    for(let i=1;i<=3;i++){
      rows.push({dia:String(d), periodo:'Noite', med:String(i), hora:'', pas:'', pad:'', pulso:''});
    }
  }
...
document.addEventListener('DOMContentLoaded', buildMRPARows);

// impressão robusta (Chrome/Windows): abre nova janela só com MRPA
function printMRPA(){
  // garante MRPA renderizado
  buildMRPARows();

  const area = document.getElementById('mrpa-print-area');
  if(!area){
    alert('Área MRPA não encontrada.');
    return;
  }
...
<body style="margin:0;padding:0">
  ...
  <script>
    // remove o botão na impressão (se existir)
    const toolbar = document.querySelector('.mrpa-toolbar');
    if(toolbar) toolbar.style.display = 'none';

    window.onload = () => {
      window.focus();
      window.print();
      // window.close(); // se quiser fechar automático, descomente
    };
  <\/script>
</body>
</html>
